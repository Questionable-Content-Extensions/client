 ...
 */
export function rgbToHsl(r: number, g: number, b: number): HSLValue {
    r /= 255
    g /= 255
    b /= 255
 ...
 */
export function hslToRgb(h: number, s: number, l: number): RGBValue {
    let r
    let g
    let b
 ...
 */
export function rgbToHsv(r: number, g: number, b: number): HSVValue {
    r = r / 255
    g = g / 255
    b = b / 255
    const max = Math.max(r, g, b)
...
export function getRgbRelativeLuminance(
    r: number,
    g: number,
    b: number
): number {
...
export function createTintOrShade(
    hexColor: string,
    iterations?: number
): string {
