...
export class ItemService {
  ...
  async getItemData(itemId: number): Promise<?DecoratedItemData> {
    const itemDataResponse = await this.$http.get(
      constants.itemDataUrl + itemId
    );
    if (itemDataResponse.status === 200) {
      const itemData = (itemDataResponse.data: DecoratedItemData);

      itemData.highlightColor = this.colorService.createTintOrShade(
        itemData.color
      );

      const friendDataRequest = this.$http.get(
        constants.itemFriendDataUrl + itemId
      );
      const locationDataRequest = this.$http.get(
        constants.itemLocationDataUrl + itemId
      );
      let imageDataRequest = null;
      if (itemData.hasImage) {
        imageDataRequest = this.$http.get(
          constants.itemDataUrl + itemId + "/images"
        );
      }
      ...
      const [friendDataResponse, locationDataResponse, imageDataResponse] =
        await Promise.all([
          friendDataRequest,
          locationDataRequest,
          imageDataRequest,
  ...
  async updateProperty(id: number, property: string, value: any) {
    ...
    try {
      const response = await this.$http.post(
        constants.setItemDataPropertyUrl,
        data
      );
      if (response.status === 200) {
        return true;
  ...
  async uploadImage(itemId: number, imageBlob: Blob, fileName: string) {
    ...
    const response = await this.$http.post(
      constants.itemDataUrl + "image/upload",
      formData,
      { contentType: undefined, dataTransform: (d) => d }
    );
    if (response.status === 200) {
      return true;
    } else {
