...
export default class DomModifier {
  modify() {
    ...
    $("title").replaceWith(
      '<title ng-controller="titleController as t">' + "{{t.title}}</title>"
    );
    ...
    $($("body #comicnav").get(0)).replaceWith(
      '<qc-nav random-comic="randomComic" main-directive="true"></qc-nav>'
    );
    $("body #comicnav").replaceWith(
      '<qc-nav random-comic="randomComic" main-directive="false"></qc-nav>'
    );
    ...
    if (comicDirective.parent().siblings(".small-2").length === 0) {
      ...
      comicDirective
        .parent()
        .after('<div class="small-2 medium-expand column"></div>');
    }
    comicDirective
      .parent()
      .siblings(".small-2")
      .prepend("<qc-extra></qc-extra>");
